...
```{r}
table(USDA$HighSodium) # 計算USDA下鹽分數量
```

```{r}
table(USDA$HighSodium, USDA$HighFat) # 計算在USDA下鹽分與脂肪兩分類的數量
```

```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE) #  在USDA下依據蛋白質計算鐵質的平均值且忽略遺漏值NA
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, max, na.rm=TRUE) # 在USDA下依據碳水化合物計算維他命的最大值且忽略遺漏值NA
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, summary, na.rm=TRUE) # 在USDA下依據碳水化合物計算維他命的基本敘述統計資料且忽略遺漏值NA
```
<br>
