...
```{r}
WHO = read.csv("data/WHO.csv") #194個資料，13個欄位
```
...
```{r}
summary(WHO) #資料檢視 (統計量)
```
*crtl + Enter 為單獨執行一行程式
*crtl+alt+i 為新增一列灰色程式區
...
```{r}
mean(WHO$Under15)  #告訴最小值的位置
sd(WHO$Under15) #告訴最小值的國家名稱
summary(WHO$Under15)
```
...
par(cex=0.8, mar=c(4,4,2,1))  #改變圖的屬性
plot(WHO$GNI, WHO$FertilityRate)  #先x軸在y軸
abline(v=10000, h=2.5, col='red')  #輔助線 v垂直線、h水平線
```

##### 資料篩選 Subsetting `subset()`. `nrow()`
```{r}
Outliers = subset(WHO, GNI > 10000 & FertilityRate > 2.5) #資料選取，掛號裡面為條件索引，選出此條件後設為outlier
nrow(Outliers) # nrow為知道outlier的共有幾行row
Outliers[c("Country","GNI","FertilityRate")] #找出outlier中的country GNI FetilityRate三個欄位
```
...
table(WHO$Region) %>% barplot(las=2, main="No. Country in Each Region") #barplot意思為長條圖 #las=2(小標國家轉90度) #main名稱
```
<br>
...
hist(WHO$CellularSubscribers) # #直方圖是數量的變數，x為連續的分布
```
...
par(mar=c(10,3,4,2),mfrow=c(1,2), cex=0.8) #各類別的數量分布 各區域的國家壽命 盒狀圖比較全面 mfow 一列兩個圖
...
##### 分類統計功能 `tapply()`
```{r}
tapply(WHO$Over60, WHO$Region, mean) #數字向量:輛別向量:平均值 分區去看 樞紐分析 依據國家去做國家的平均值
```
mean oh over 15 by rigion
...
```{r}
USDA = read.csv("data/USDA.csv")  #此行程式用於載資料
summary(USDA) #看基本統計量
```
<br>
...
&emsp; ■ 盒狀圖和直方圖有何異同？<br> 異:盒狀圖能看出準確的中位數、1Q、3Q、Range、最大值、最小值，直方圖則較難看出 
                                      ；同:兩種皆是數量的連續分布，且直方圖若傾倒90度就是盒狀圖
                                      
&emsp; ■  直方圖的Y軸應該用什麼標題呢？<br> Frenquency
</p class="qiz">
<br><br><hr>
...
USDA$HighSodium = as.numeric(USDA$Sodium > mean(USDA$Sodium, na.rm=TRUE)) #as.numeric將掛號內的邏輯向量轉成數值
...
```{r}
table(USDA$HighSodium) #計算高鹽份、低鹽份的數量，true=Highsodium=1,false=Lowsodium=0
```

```{r}
table(USDA$HighSodium, USDA$HighFat) #計算高鹽份、低鹽份、高脂肪、低脂肪的數量,true=Highsodium、HighFat=1,false=Lowsodium、LowFat=0
```

```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE) #依據蛋白質計算鐵質的平均值且忽略遺漏值NA
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, max, na.rm=TRUE) #依據碳水化合物計算維他命的最大值且忽略遺漏值NA
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, summary, na.rm=TRUE) #依據碳水化合物計算維他命的基本敘述統計資料且忽略遺漏值NA
```
<br><br>
