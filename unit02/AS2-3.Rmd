...
#Native Citizenship=0.88833、Naturalized＝0.05387
```
...
#White、Black、Multiracial、American Indian
```
<br><br><hr>
...
```{r}
table(is.na(A$MetroAreaCode), A$State)
#Alaska、Wyoming
```
...
abs(0.3 - tapply(is.na(A$MetroAreaCode), A$State, mean)) %>% sort %>% head
#Wisconsin
```

Which state has the largest proportion of non-metropolitan interviewees, ignoring states where all interviewees were non-metropolitan?
```{r}
tapply(is.na(A$MetroAreaCode), A$State, mean) %>% sort %>%tail
#Alaska、Wyoming
```
...
#首訪者中有0.3087的比率來自“New York-Northern New Jersey-Long Island, NY-NJ-PA"
```
...
#'Detroit-Warren-Livonia'在India中有最大值的
```

In Brazil?
```{r}
tapply(A$Country=="Brazil", A$MetroArea, sum) %>% sort %>% tail
#‘Boston-Cambridge-Quincy’在Brazil中有最大值的受訪
```

In Somalia?
```{r}
tapply(A$Country=="Somalia", A$MetroArea, sum) %>% sort %>% tail
#Phoenix-Mesa-Scottsdale,St. Cloud在Somalia中有最大值的受訪者
```
