...
請算出 白種女生總共有多少人
```{r}
sum(skin_color=='white'&isMale==FALSE)
```
...
```{r}
df[df$height > 180 , "name"] # 身高高於180的名字
```

```{r}
subset(df, height > 170 & !isMale)$name # 選取身高高於170的女生名字
```

```{r}
mean(df$height[df$isMale]) # 男生的平均身高
```

```{r}
df$height[!df$isMale] %>% mean # 所有女生的平均身高
```

```{r}
sum( subset(df, !isMale)$noBuy ) # 女性總共買了多少次
```

```{r}
subset(df, skin_color == "white" & !isMale ) %>% nrow # 選取白人女性的列個數
```

```{r}
sum(df$skin_color == "white" & !df$isMale ) # 白人女性的總人數
```
